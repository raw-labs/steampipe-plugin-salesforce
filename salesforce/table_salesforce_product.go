...
func SalesforceProduct(ctx context.Context, dm dynamicMap, config salesforceConfig) *plugin.Table {
	...
	return &plugin.Table{
		...
		Columns: mergeTableColumns(ctx, config, dm.cols, []*plugin.Column{
			...
			{Name: "created_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date and time of the creation of the product role record.", Sort: plugin.SortAll},
			...
			{Name: "last_referenced_date", Type: proto.ColumnType_TIMESTAMP, Description: "The timestamp when the current user last viewed product record.", Sort: plugin.SortAll},
			{Name: "last_viewed_date", Type: proto.ColumnType_TIMESTAMP, Description: "The timestamp when the current user last viewed this record. If this value is null, this record might only have been referenced (last_referenced_date) and not viewed by the current user.", Sort: plugin.SortAll},
