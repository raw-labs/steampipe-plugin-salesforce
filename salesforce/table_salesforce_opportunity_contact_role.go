...
func SalesforceOpportunityContactRole(ctx context.Context, dm dynamicMap, config salesforceConfig) *plugin.Table {
	...
	return &plugin.Table{
		...
		Columns: mergeTableColumns(ctx, config, dm.cols, []*plugin.Column{
			...
			{Name: "created_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date and time of the creation of the contact role record.", Sort: plugin.SortAll},
			...
			{Name: "last_modified_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date of most recent change in the contact role record.", Sort: plugin.SortAll},
		}),
	}
}
