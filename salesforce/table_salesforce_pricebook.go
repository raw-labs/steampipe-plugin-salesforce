...
func SalesforcePricebook(ctx context.Context, dm dynamicMap, config salesforceConfig) *plugin.Table {
	...
	return &plugin.Table{
		...
		Columns: mergeTableColumns(ctx, config, dm.cols, []*plugin.Column{
			...
			{Name: "last_modified_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date of most recent change in the product record.", Sort: plugin.SortAll},
			{Name: "last_referenced_date", Type: proto.ColumnType_TIMESTAMP, Description: "The timestamp for when the current user last viewed a record related to this record.", Sort: plugin.SortAll},
			{Name: "last_viewed_date", Type: proto.ColumnType_TIMESTAMP, Description: "The timestamp for when the current user last viewed this record. If this value is null, it's possible that this record was referenced (LastReferencedDate) and not viewed.", Sort: plugin.SortAll},
