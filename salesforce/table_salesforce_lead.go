...
func SalesforceLead(ctx context.Context, dm dynamicMap, config salesforceConfig) *plugin.Table {
	tableName := "Lead"
	return &plugin.Table{
		...
		Columns: mergeTableColumns(ctx, config, dm.cols, []*plugin.Column{
			...
			{Name: "converted_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date on which this lead was converted.", Sort: plugin.SortAll},
			{Name: "created_by_id", Type: proto.ColumnType_STRING, Description: "Id of the user who created the lead."},
			{Name: "created_date", Type: proto.ColumnType_TIMESTAMP, Description: "Creation date and time of the lead.", Sort: plugin.SortAll},
			...
			{Name: "last_modified_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date and time of the last changes to lead record.", Sort: plugin.SortAll},
