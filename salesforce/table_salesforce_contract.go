...
func SalesforceContract(ctx context.Context, dm dynamicMap, config salesforceConfig) *plugin.Table {
	...
	return &plugin.Table{
		...
		Columns: mergeTableColumns(ctx, config, dm.cols, []*plugin.Column{
			...
			{Name: "end_date", Type: proto.ColumnType_TIMESTAMP, Description: "Calculated end date of the contract. This value is calculated by adding the ContractTerm to the start_date.", Sort: plugin.SortAll},
			{Name: "owner_id", Type: proto.ColumnType_STRING, Description: "ID of the user who owns the contract."},
			{Name: "start_date", Type: proto.ColumnType_TIMESTAMP, Description: "Start date for this contract.", Sort: plugin.SortAll},
			...
			{Name: "activated_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date and time when this contract was activated.", Sort: plugin.SortAll},
			{Name: "billing_address", Type: proto.ColumnType_JSON, Description: "Billing address of the account."},
			{Name: "company_signed_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date on which the contract was signed by organization.", Sort: plugin.SortAll},
			...
			{Name: "created_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date and time when contract record was created.", Sort: plugin.SortAll},
			{Name: "customer_signed_date", Type: proto.ColumnType_TIMESTAMP, Description: "Date on which the customer signed the contract.", Sort: plugin.SortAll},
			...
			{Name: "last_activity_date", Type: proto.ColumnType_TIMESTAMP, Description: "Value is one of the following, whichever is the most recent. a) Due date of the most recent event logged against the record. b) Due date of the most recently closed task associated with the record.", Sort: plugin.SortAll},
			{Name: "last_approved_date", Type: proto.ColumnType_TIMESTAMP, Description: "Last date the contract was approved.", Sort: plugin.SortAll},
			...
			{Name: "last_modified_date", Type: proto.ColumnType_TIMESTAMP, Description: "The date and time of the last change to contract record.", Sort: plugin.SortAll},
			...
			{Name: "last_viewed_date", Type: proto.ColumnType_TIMESTAMP, Description: "The timestamp when the current user last viewed this record or list view. If this value is null, the user might have only accessed this record or list view (last_referenced_date) but not viewed it.", Sort: plugin.SortAll},
			...
			{Name: "system_modstamp", Type: proto.ColumnType_TIMESTAMP, Description: "The date and time when contract was last modified by a user or by an automated process.", Sort: plugin.SortAll},
		}),
	}
}
